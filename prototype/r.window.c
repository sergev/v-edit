 ...
 */
void win_create(window_t *w, int col_left, int col_right, int row_top, int row_bottom,
                int need_borders)
{
    register int i, size;
...
void win_open(char *file)
{
     ...
     */
    if (cursorcol == 0 && cursorline > 0 && cursorline < oldwin->text_maxrow) {
        /* Split horizontally. */
        register int i;
        win_create(win, oldwin->base_col, oldwin->max_col, oldwin->base_row + cursorline + 1,
                   oldwin->max_row, 1);
    ...
    } else if (cursorline == 0 && cursorcol > 0 && cursorcol < oldwin->text_maxcol - 1) {
        /* Split vertically. */
        register int i;
        win_create(win, oldwin->base_col + cursorcol + 2, oldwin->max_col, oldwin->base_row,
                   oldwin->max_row, 1);
     ...
     */
    win_create(win, oldwin->base_col, oldwin->max_col, oldwin->base_row, oldwin->max_row, 1);
#endif
...
void win_remove()
{
    ...
    if (pwin->max_row != win->max_row) {
        ...
        for (i = 0; i <= pwin->text_maxrow; i++) {
            pwin->lastcol[i] = win->lastcol[i] + win->base_col - pwin->base_col;
    ...
    for (i = 0; i < nwinlist; i++) {
        ...
        if (tw->wfile == fn && tw != w) {
            ...
            if (j <= k) {
                ...
                drawlines(j - tw->topline, delta == 0 ? k - tw->topline : winlist[i]->text_maxrow);
                win_switch(owin);
                poscursor(l, m);
            }
        }
