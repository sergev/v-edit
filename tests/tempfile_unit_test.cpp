...
TEST_F(TempfileTest, WriteLineToTempMultiple)
{
    ...
    EXPECT_EQ(s3.sizes[0], 11); // "Third line\n" = 10 + 1 = 11 bytes
}
...
TEST_F(TempfileTest, WriteLinesToTempMixedNewlines)
{
    ...
    EXPECT_EQ(seg.sizes[2], 1);  // "\n"
    EXPECT_EQ(seg.sizes[3], 15); // "Another normal\n"
}
...
TEST_F(TempfileTest, WriteLinesToTempManyLines)
{
    ...
    for (int i = 0; i < 100; ++i) {
        ...
        EXPECT_EQ(seg.sizes[i], line.size());
    }
}
...
TEST_F(TempfileTest, WriteLinesToTempPositionTracking)
{
    ...
    EXPECT_EQ(s1.sizes[1], 13); // "Block1 Line2\n"
    EXPECT_EQ(s2.sizes[0], 12); // "Single line\n"
    EXPECT_EQ(s3.sizes[0], 13); // "Block2 Line1\n"
}
