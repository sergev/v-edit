...
void drawlines(int lo, int lf)
{
    ...
    if (lo > 0)
        lo = 0; /* Initial column */
    l1          = lo;
    lmc         = (curwin->base_col == curwin->text_col ? 0 : curwksp->offset == 0 ? LMCH : MLMCH);
...
void wputc(int j, int flg)
{
    ...
    putch(j);
    if (cursorcol <= 0)
        poscursor(0, cursorline < 0 ? 0 : cursorline > curwin->text_maxrow ? 0 : cursorline);
    movecursor(0);
}
...
back:
    ...
    if (cline >= curwin->text_row && cline <= curwin->text_maxrow && ccol >= curwin->text_col &&
        ccol <= curwin->text_maxcol) {
