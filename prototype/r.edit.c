...
static void cline_setup(int fi, int offset)
{
    ...
    if (file_desc == fi && offset <= file_offset && offset >= file_offset - buf_count) {
...
void insertlines(int from, int number)
{
    ...
    wksp_redraw((workspace_t *)NULL, curfile, from, from + number - 1, number);
...
void checksegm()
{
    ...
    while (f) {
        if (curwksp->line >= nl && curwksp->line < nl + f->nlines && curwksp->cursegm != f &&
            curwksp->cursegm->prev)
