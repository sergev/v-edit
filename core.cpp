...
void Editor::startup(int restart)
{
    ...
    if (has_colors()) {
        ...
        init_pair(int(Color::POSITION), COLOR_WHITE, COLOR_RED);    // Cursor position color pair
        init_pair(int(Color::TRUNCATION), COLOR_WHITE, COLOR_BLUE); // Truncation color pair
